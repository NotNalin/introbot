...
step = {
    0: "New to the GTech μLearn Discord server? Finished setting up your profile? Well then, a hearty welcome to μLearn Discord server 🎉\nVisit mention_channel channel to see the welcome card we've prepared just for you!",
    1: "Welcome to GTech MuLearn! Do follow the mention_channel for a smooth sail ahead ⚠\nVisit the mention_channel channel to know more about the Do-s and Don't-s.",
    2: "Keen to follow up the activities of μLearn? We've compiled the schedules of all our current and upcoming events just for you 📣\nHead on to the mention_channel channel for all our updates!",
    3: "Ever thought of acquiring a job through μLearn? You'll be glad to know that not just students, even companies onboard GTech μLearn in search of talent 💫\nVisit mention_channel to keep track of announcements and opportunities from our partner companies.",
    4: "Tasks are allotted according to your current level in the μLearn Discord server. Visit the channel corresponding to your current level to know more about the tasks and their corresponding Karma Points ✨",
    5: "Hello µLearner! Great to have you onboard. Eager to know about your peers? Start by posting an introduction about yourself 🙋\nHead on to mention_channel channel to meet your peers and introduce yourself to them!",
    6: "Karma Point is a digital score system implemented at μLearn. Each task is associated with a specific amount of Karma Points that can be earned once the proof of completion of that task has been submitted 🪙Buckle up and mine Karma Points and climb all the way up the leaderboard! Here is a task for you\n\nMy Muid\n Post your muid with the hashtag '#my-muid' in this channel\nEg: #my-muid ousu@mulearn",
    7: "μLearn follows a peer approval system, which means all tasks submitted by you are reviewed by none other than your peers!\nThe reviewers are specially assigned for this role and their task is to flag your proof of work according to its validity.\n• 🚩 - A red flag means your task is ineligible to get Karma points due to some reason.\n• 🏁 - A checkered flag means your task is eligible and Karma Points will be awarded shortly.\nSo next time you submit a task, pay heed to its peer approval status and do rectify errors if any 😇",
    8: "Once you've submitted a task and your proof of work has been reviewed, the next step is awarding of Karma Points!\n• ✅ - A green tick box means your task has been awarded Karma Points.\nAll tasks that have been appraised as shown above are automatically rewarded Karma Points ✨",
    9: "Once your task has been verified and rewarded Karma Points, you'll be notified about the same in your Discord DM by our bot 💫\nmention_channel can be used to keep track of all the tasks for which you've earned Karma Points systematically!",
    10: "What if I said you can earn Karma points just by chatting? μLearn now awards you Chat Karma for connecting with your peers in the respective text channel 🥳\nMaintain your streak and walk away with a daily bundle of Karma Points just by chatting!",
    11: "Who does not like a bit a competition amongst their peers? Your Rank tells you how far you're ahead of your peers. Maximize your Karma mining and don't let the others topple you on your way to the leaderboard 🔥\nVisit the mention_channel channel and use the /rank command to see where you stand among your fellow learners!",
    12: "Have any doubts regarding the task? Were your Karma Points not allotted properly? Is there any error in your rank? Facing any further other issue at μLearn Discord server? Worry not! We're always here to help 😌\nAll you got to do is raise a support ticket using the command /support-ticket in any of our text channels and our moderators will be in touch shortly to resolve all your current issues!",
    13: "Need real-time help or support in our sever? Tag the Discord Moderator by selecting @Discord Moderator from the pop in any available text channel.\nWe'll make sure your issue gets resolved as soon as possible "
}
...
class IntroCog(Cog):
    ...
    async def intro(self, interaction: discord.Interaction):
        ...
        for welcome in guild.channels:
            if welcome.name == 'welcome':
                break
        await channel.send(step[0].replace("mention_channel", welcome.mention))
        await channel.send("NB: Please return back to intro channel")
        await asyncio.sleep(3600)
        await channel.delete()
        self.intro_queries.delete_log(interaction.user.id)

    async def check_msg(self, message):
        user_id = self.intro_queries.fetch_user_id_from_discord_id(message.author.id)
        if self.intro_queries.is_valid_channel(message.channel.id, user_id):
            order = self.intro_queries.check_step_order(user_id)
            if order == 7:
                if not await self.peer_approve(message):
                    return
            if order == 8:
                task_message_id = self.intro_queries.fetch_task_message_id(user_id)
            ...
            if order == 14:
                ...
                await message.author.send("You have successfully completed the intro task. Here is your certificate")
            ...
            self.intro_queries.update_progress(user_id, order + 1)
